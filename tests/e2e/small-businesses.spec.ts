...
test.describe("Small Businesses page", () => {
	test("loads with 200 status", async ({ page }) => {
		const response = await page.goto("/small-businesses/")
	...
	test("H1 exists and contains expected text", async ({ page }) => {
		...
		expect(text).toContain("Protecting your business")
		expect(text).toContain("complicated")
	})

	test("has Common Misconceptions section with 4 cards", async ({ page }) => {
		await page.goto("/small-businesses/")

		const h2 = page.locator("h2", { hasText: "Common Misconceptions" })
	...
	test("misconception cards address all four myths", async ({ page }) => {
		await page.goto("/small-businesses/")
		...
		for (const myth of myths) {
			const card = page.locator(".card", { hasText: myth })
			await expect(card, `Card about "${myth}" should exist`).toHaveCount(1)
		}
	})
